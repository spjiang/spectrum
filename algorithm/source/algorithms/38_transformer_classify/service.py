...
async def run(*, file: UploadFile, file2: UploadFile | None, params_json: str):
    """file=Cube，file2=标签。"""
    if file2 is None:
        return err_response(algorithm_id=ALGORITHM_ID, algorithm=TITLE, message="Transformer 分类需要 file2 标签 GeoTIFF")
    ...
    save_preview_png(pred_map.astype(float), png, title="Transformer Pred")
    return ok_response(
        algorithm_id=ALGORITHM_ID,
        algorithm=TITLE,
        implemented=True,
        message="SpectralFormer（Hong 2022）分类完成",
        data={**result, "epochs": epochs, "format": "GeoTIFF", "backend": "PyTorch SpectralFormer"},
