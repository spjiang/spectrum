...
OUT = ROOT / "docs" / "高光谱算法体系-培训汇报.pptx"
...
ALG_IDS = [
    ...
    "46_reci", "47_gndvi", "48_osavi", "49_arvi", "50_vari",
    "51_lai_index", "52_nbr", "53_sipi", "54_gci", "55_ndsi",
]
...
ENRICH = {
    ...
    3: {
        "method": "读取已解算 POS CSV，做粗差剔除、轨迹平滑与杠杆臂校正，输出平滑后的位置、姿态与速度。",
        "value": "为后续粗定位与正射提供更稳定的轨迹，而不是从原始 IMU 重新解算姿态。",
        "risk": "无可用 POS CSV 则无法平滑轨迹或改正安装偏移，后续几何处理缺少外方位输入。",
        "pipeline": "L0 原始包 → POS 轨迹平滑/杠杆臂 → #15/#16 几何校正",
        "related": "关联：#2 时间对齐、#15 地理定位、#16 正射",
        "tip": "输入须已含 time/lat/lon/alt/roll/pitch/yaw；本服务不读取原始 IMU 比力或角速度。",
    },
    4: {
        "method": "统计过曝/欠曝比例与场景亮度分布，仅以过曝比例门控通过与复飞。",
        "value": "在处理管线入口拦截过曝架次，控制无效算力投入与返工风险。",
        "risk": "劣质数据进入后续环节，将造成指数/分类结果不可信并浪费处理资源。",
        "pipeline": "落地 → 过曝质检闸门 → 通过则进入 L0→L1；不通过则补飞",
        "related": "关联：#1 航线规划、#5 云影检测、#10 辐射定标",
        "tip": "建议将饱和像元比例设为硬阈值，写入项目验收标准；本服务不检测丢帧。",
    },
    5: {
    ...
    15: {
        "method": "以单个 POS 中心点与估算 GSD 写北向上仿射，供落点预览；不使用姿态。",
        "value": "快速确认飞行落点，为精正射提供粗定位起点。",
        "risk": "无法预览落点时，任务质检与正射排错效率显著下降。",
        "pipeline": "影像条带 + POS 中心点 → 粗地理参考 → #16 正射",
        "related": "关联：#3 POS、#16 正射、#4 质检",
        "tip": "精度通常不足以直接量亩，也不是共线方程正射，但满足任务级预览。",
    },
    16: {
    ...
    17: {
        "method": "把同 CRS 的两幅 GeoTIFF 落到统一网格，重叠区做距离加权羽化；不搜索接缝线，也不做同名点匹配。",
        "value": "形成测区完整 L2 产品，供整村/整园尺度分析与制图。",
        "risk": "仅保留碎片条带时，无法开展区域级专题产品。",
        "pipeline": "多条正射条带 → 地理镶嵌/羽化 → 整景反射率正射立方体（典型 L2）",
        "related": "关联：#16 正射、#18 匀色、#19 配准",
        "tip": "算法研发侧最常见的标准输入形态。",
    },
    18: {
        "method": "逐波段执行 Wallis 局部匀色；当前不搜索或优化接缝线。",
        "value": "提升交付图可读性与验收通过率，不改变几何框架。",
        "risk": "明暗跳变会降低产品观感，影响业务验收评价。",
        "pipeline": "初镶嵌 → Wallis 局部匀色 → 交付或进入分析",
        "related": "关联：#17 镶嵌、#11 相对辐射",
        "tip": "偏交付质量环节，但对项目验收影响直接。",
    },
    19: {
        "method": "估计一个全局亚像元平移，把 RGB 栅格对齐到 HSI；不处理矢量。",
        "value": "支撑「真彩色目视 + 光谱算法 + 按地块统计」的标准业务形态。",
        "risk": "未配准导致标注错位、裁剪错误，面积与专题统计失效。",
        "pipeline": "HSI + RGB → 全局平移配准 → 标注/统计",
        "related": "关联：#17 镶嵌、#34–40 分析、#45 汇总",
        "tip": "打通「图、谱、田」三层数据是产品闭环关键。",
    },
    20: {
        "method": "按场景像元均值/标准差比（非传感器 SNR）与水汽吸收特征剔除坏波段；本算法只剔除波段，不执行光谱去噪。",
    ...
    21: {
        ...
        "related": "关联：#20 坏波段剔除、#35 SAM、#32 回归",
    ...
    33: {
        "method": "PROSAIL 查找表，默认 RMSE，对最优若干条的 LAI/Cab 取平均。",
        "value": "不用化验图，一次给出叶面积和叶绿素两张图，用来分清是叶子少还是叶子黄。",
        "risk": "土壤、叶倾角等写死；边界命中不是实测极值，结果也不是化验值。",
        "pipeline": "反射率 + 波长 + 几何 → LUT 匹配平均 → lai.tif / cab.tif",
        "related": "关联：#13 大气校正、#32 经验回归",
        "tip": "不要化验图，但要反射率和采集几何；与 #32 互补。",
    },
    34: {
    ...
    36: {
        "method": "1D-CNN 光谱分类；RNN 未实现。卷积沿光谱维提取局部特征后完成像素分类。",
        "value": "提供仅使用单像元光谱的一维卷积分类教学实现。",
        "risk": "默认短训且按像元随机划分；不使用空间邻域，也不能声称具备 RNN 能力。",
    ...
    38: {
        "method": "SpectralFormer 光谱分类；GCN 未实现。相邻波段 token 经缩小 Transformer 编码器分类。",
        "value": "提供光谱序列长程依赖的教学分类实现。",
        "risk": "当前无 GCN、空间分支或预训练，短训指标不能外推。",
        "pipeline": "单像元光谱 → 相邻波段 token → Transformer → 分类图",
        "related": "关联：#22 前处理、#36/#37 CNN、#39 少样本",
        "tip": "训练与推理必须保持波段顺序和预处理一致。",
    },
    39: {
        "method": "SAM 均值原型少样本分类；迁移学习未实现。每类支持光谱求均值后按最小光谱角分类。",
        "value": "用少量已标注支持像元构造本景均值原型并生成分类图。",
        "risk": "没有可学习嵌入、源域预训练、迁移学习或未知类拒绝。",
        "pipeline": "本景标签 → 每类均值原型 → SAM 最近原型 → 分类图",
        "related": "关联：#35 SAM、#26 样本、#45 汇总",
        "tip": "支持点须覆盖类内差异，并与查询区空间分离。",
    },
    40: {
        "method": "低 NDVI 种子 ACE 目标检测；语义分割未实现。由低 NDVI 候选构造目标光谱并阈值化 ACE 得分。",
        "value": "输出本景相对目标相似度、候选掩膜和连通斑块矢量。",
        "risk": "低 NDVI 种子是仓库启发式；ACE 高分不是概率、病害确认或决策依据。",
        "pipeline": "反射率 Cube → 低 NDVI 种子 → ACE 得分 → 掩膜/候选斑块",
        "related": "关联：#27 NDVI、#42 异常检测、#45 汇总",
        "tip": "结果只用于候选复核，不生成处方或业务决策。",
    },
    41: {
    ...
    46: {
        "method": "RECI = NIR/RE − 1，须有真实红边通道。",
        "value": "密冠层叶绿素相关相对分区，与 NDRE 互补。",
        "risk": "无真红边时同名指数没有物理意义。",
        "pipeline": "L2 红边+近红外 → reci.tif → 相对对照 / #45",
        "related": "关联：#28 NDRE、#54 GCI",
        "tip": "值域不是 −1～1；不能写成叶绿素毫克数。",
    },
    47: {
        "method": "GNDVI = (NIR − GREEN)/(NIR + GREEN)。",
        "value": "绿光通道的相对绿度，波段条件与 NDVI 同类。",
        "risk": "绿光窗口不同则不能横比；高覆盖仍可能饱和。",
        "pipeline": "L2 绿光+近红外 → gndvi.tif → #45",
        "related": "关联：#27 NDVI、#54 GCI",
        "tip": "按传感器光谱响应选绿光，不要死记教学默认 1。",
    },
    48: {
        "method": "OSAVI = (NIR − RED)/(NIR + RED + L)，默认 L=0.16。",
        "value": "稀疏植被时压土壤亮度的相对绿度。",
        "risk": "改 L 后不能与未改图横比；不是叶面积。",
        "pipeline": "L2 红光+近红外 → osavi.tif → #45",
        "related": "关联：#29 SAVI/MSAVI",
        "tip": "L=0 即 NDVI 形式；0.16 是文献常用值。",
    },
    49: {
        "method": "ARVI = (NIR − RB)/(NIR + RB)，RB = RED − γ(BLUE − RED)，默认 γ=1。",
        "value": "气溶胶残差可见时的相对绿度对照。",
        "risk": "不能替代大气校正；蓝光差时比 NDVI 更乱。",
        "pipeline": "L2 蓝/红/近红外 → arvi.tif → 对照",
        "related": "关联：#13 大气校正、#27 NDVI",
        "tip": "γ=1 不是本相机现场标定。",
    },
    50: {
        "method": "VARI = (GREEN − RED)/(GREEN + RED − BLUE)，只用可见光。",
        "value": "没有近红外时的相对覆盖示意。",
        "risk": "不是覆盖度百分数，也不能替代 NDVI。",
        "pipeline": "可见光三通道 → vari.tif",
        "related": "关联：#27 NDVI",
        "tip": "未定标 JPEG 不能声称物理 VARI。",
    },
    51: {
        "method": "经验 LAI = max(3.618×EVI − 0.118, 0)，不是 #33 PROSAIL。",
        "value": "指数菜单里的教学叶面积示意。",
        "risk": "线性系数未经本地标定；产物不要和 #33 的 lai.tif 混。",
        "pipeline": "L2 蓝/红/近红外 → EVI → lai_index.tif",
        "related": "关联：#29 EVI、#33 物理反演",
        "tip": "负值裁零不表示真实零叶面积。",
    },
    52: {
        "method": "NBR = (NIR − SWIR)/(NIR + SWIR)；教学 SWIR 约 1600 nm。",
        "value": "过火前后的相对差异对照。",
        "risk": "与 NDMI 同型不同用途；无 SWIR 无物理意义。",
        "pipeline": "L2 近红外+SWIR → nbr.tif",
        "related": "关联：#30 NDMI",
        "tip": "Landsat 过火产品常用 SWIR2 约 2.1 μm，不能混比。",
    },
    53: {
        "method": "SIPI = (NIR − BLUE)/(NIR − RED)。",
        "value": "色素比值相关相对差异。",
        "risk": "分母接近零会发炸；不是色素毫克数。",
        "pipeline": "L2 蓝/红/近红外 → sipi.tif",
        "related": "关联：#27 NDVI",
        "tip": "值域不必在 −1～1。",
    },
    54: {
        "method": "GCI = NIR/GREEN − 1。",
        "value": "绿光叶绿素相关相对分区，不要求红边。",
        "risk": "不能直接当作叶绿素含量。",
        "pipeline": "L2 绿光+近红外 → gci.tif",
        "related": "关联：#46 RECI、#47 GNDVI",
        "tip": "与 RECI 同型，分母是绿光。",
    },
    55: {
        "method": "NDSI = (GREEN − SWIR)/(GREEN + SWIR)。",
        "value": "积雪相对格局示意。",
        "risk": "与 MNDWI 同型不同用途；不输出二值雪图。",
        "pipeline": "L2 绿光+SWIR → ndsi.tif",
        "related": "关联：#30 MNDWI",
        "tip": "无真 SWIR 不要用末波段冒充。",
    },
}
...
def footer(slide, page: int, total: int, section: str = ""):
    ...
    add_text(slide, 10.5, 7.2, 2.5, 0.28, f"{page} / {total}", size=10, color=C["white"], align=PP_ALIGN.RIGHT)


def title_header(slide, title: str, subtitle: str = "", pill: str = ""):
    ...
    if pill:
        add_round(slide, 11.05, 0.22, 1.9, 0.42, C["warm"])
        add_text(slide, 11.05, 0.26, 1.9, 0.35, pill, size=11, bold=True, color=C["white"], align=PP_ALIGN.CENTER)
...
def build():
    ...
    assert len(algos) == 55, f"期望 55 项，实际 {len(algos)}"
    ...
    add_text(
        s, 0.8, 2.55, 11.5, 1.2,
        [
            ("架构设计  ·  55 项算法详解  ·  服务能力与对接", 18, False, (167, 243, 208)),
            ("正式培训与业务汇报材料", 14, False, (148, 163, 184)),
        ],
    )
    ...
    add_text(
        ...
        [
            ...
            ("依据：algorithm/docs/采集到算法-算法清单.md", 12, False, (148, 163, 184)),
            ("配套服务：algorithm/source（HTTP API，GeoTIFF 等业界格式）", 12, False, (148, 163, 184)),
        ],
    )
    ...
    s = blank(prs)
    track(s, "目录")
    title_header(s, "目录", "从采集到地块汇总的完整能力说明")
    cards = [
        ("01  架构设计", ["数据层级 L0–L4 定义与用户", "分类链与长势指标链", "业界交换文件格式", "端到端业务处理路径"], C["primary"]),
        ("02  算法清单（55项）", ["按层级说明功能与方法要点", "明确输入输出与产品形态", "业务价值、缺失影响与流程位置", "关联算法与实施提示"], C["accent"]),
    ...
    levels = [
        ...
        ("L1 辐射校正", ["形态：辐亮度", "可含粗几何", "", "用户：处理中心", "", "决策：否", "价值：物理可比"]),
        ("L2 反射率、镶嵌与特征", ["形态：反射率", "正射立方体", "", "用户：算法研发", "", "决策：专业预览", "价值：标准输入"]),
        ("L3 指数与识别", ["形态：分类/指数", "反演/矢量", "", "用户：分析与产品", "", "决策：部分可读", "价值：信息提取"]),
        ("L4 地块汇总", ["形态：亩数报表", "告警与 API", "", "用户：业务与系统", "", "决策：是", "价值：管理闭环"]),
    ]
    ...
    s = blank(prs)
    track(s, "算法清单")
    title_header(s, "第二部分｜算法清单（55 项）", "每项说明：功能 · 方法 · 场景 · 输入输出 · 价值 · 影响 · 流程")
    section_card(s, 0.35, 1.2, 12.6, 5.6, "阅读与培训建议", [
        "1. 先按层级理解「本层解决什么业务问题」，再记忆算法名称。",
        "2. 每页包含：功能定义、方法要点、适用场景、输入输出、业务价值、缺失影响、流程位置与关联。",
        "3. 标注「可运行」表示当前 HTTP 服务可联调（教学实现）。",
        "4. 分层覆盖：",
        "   #1–5 航线规划与采集质检　#6–11 辐射校正与相对归一　#12–26 反射率、镶嵌与特征",
        "   #27–43 指数与识别　#44–45 图斑整理与地块汇总　#46–55 植被指数补齐",
        "5. 与仓库关系：L0–L2 工程能力多在处理中心；本仓库强项在 L2→L3 分类及可运行教学 API。",
    ], C["primary"], 14, 13)

    groups = [
        ("L0 航线规划与采集质检", "1–5", "飞行与原始数据质量控制", list(range(1, 6))),
        ("L0→L1 辐射校正", "6–11", "从 DN 到物理辐亮度", list(range(6, 12))),
        ("L1→L2 反射率与正射", "12–19", "反射率、正射、镶嵌、配准", list(range(12, 20))),
        ("L2 镶嵌与特征", "20–26", "清洗、特征、样本构建", list(range(20, 27))),
        ("L3 指数与识别（上）", "27–33", "长势、水分、生化量", list(range(27, 34))),
        ("L3 指数与识别（下）", "34–43", "类别、斑块、异常、变化", list(range(34, 44))),
        ("L3→L4 图斑整理与地块汇总", "44–45", "平滑与地块统计", list(range(44, 46))),
        ("L3 植被指数补齐", "46–55", "RECI 到 NDSI 的独立指数", list(range(46, 56))),
    ]
    for page_groups in (groups[:4], groups[4:]):
        s = blank(prs)
        track(s, "算法清单")
        title_header(s, "算法分层总览", "55 项能力与业务问题的对应关系")
    ...
    for a in algos:
        ...
        track(s, "算法清单")
        num = a["num"]
        pill = "可运行" if num in IMPL else ""
        title_header(s, f"{num:02d}. {a['name']}", f"所属层级：{a.get('level', '')}　｜　编号对齐算法清单与 API", pill=pill)

        # 上行三卡：功能 / 方法 / 场景
        section_card(s, 0.25, 1.08, 4.2, 2.35, "一、功能定义", [
            a["role"],
            "",
            "【输出产品】",
            a["out"],
        ...
        section_card(s, 4.55, 1.08, 4.35, 2.35, "二、方法要点", [
            a.get("method", "—"),
            "",
            a.get("tip", ""),
        ], C["accent"], 12, 11)

        section_card(s, 9.0, 1.08, 4.05, 2.35, "三、适用场景", [
            a["scene"],
            "",
            "【输入规格】",
            a["inp"],
        ...
        # 下行三卡：价值 / 影响 / 流程关联
        section_card(s, 0.25, 3.55, 4.2, 3.35, "四、业务价值", [
            a.get("value", "—"),
            "",
            "【可形成的产品】",
            a["out"],
        ...
        section_card(s, 4.55, 3.55, 4.35, 3.35, "五、缺失影响", [
            a.get("risk", "—"),
            "",
            "【质量关注点】",
            a.get("tip", "按项目验收标准设置阈值与抽检。"),
            "",
            "【影响范围】精度 · 工期 · 验收通过率",
        ], C["warm"], 12, 11)

        aid = ALG_IDS[num - 1]
        section_card(s, 9.0, 3.55, 4.05, 3.35, "六、流程位置与关联", [
            "【在链路中的位置】",
            a.get("pipeline", "—"),
            "",
            a.get("related", "—"),
            "",
            "【服务状态】",
            "已实现可联调" if num in IMPL else "目录与接口已预留（骨架）",
            "",
    ...
    s = blank(prs)
    track(s, "服务对接")
    title_header(s, "当前已实现能力（可联调）", "55 项全部可运行；接口形状按业界格式对齐")
    section_card(s, 0.35, 1.2, 12.6, 5.6, "可运行 55 项", [
        "L0–L2：航线、质检、定标、正射、镶嵌与特征",
        "L3 指数：#27–30 与补齐的 #46–55",
        "L3 识别：红边参数、反演、分类、检测、解混、变化",
        "L4：图斑平滑与地块分区统计",
        "",
        "冒烟：scripts/smoke_all_implemented.py 为 55/55。",
        "详细 curl 见：docs/算法API测试清单.md",
    ], C["primary"], 13, 12)
    ...
    section_card(s, 0.35, 1.2, 12.6, 5.6, "本材料覆盖的三部分", [
        "1. 架构：L0→L4 分层清晰，分类链与指标链并行。",
        "2. 算法：55 项业界清单，逐项说明方法、输入输出、价值与流程位置。",
        "3. 服务：HTTP 统一入口，GeoTIFF/GeoJSON 正规对接，可培训可联调。",
        "",
        "建议下一步：",
        "① 选定 1–2 个业务场景（作物分类 / 长势监测）打通应用演示。",
        "② 按场景优先级补齐关键骨架算法到生产级。",
        "③ 与 hyper-spectral-small-modes 分类模型深度对接本服务。",
    ], C["primary"], 14, 13)
