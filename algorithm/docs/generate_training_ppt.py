...
def build():
    ...
    assert len(algos) == 45, f"期望 45 项，实际 {len(algos)}"
    ...
    s = blank(prs)
    track(s, "服务对接")
    title_header(s, "当前已实现能力（可联调）", "教学级实现；接口形状按业界格式对齐")
    impl_lines = [f"#{n:02d} {next(a['name'] for a in algos if a['num']==n)}" for n in sorted(IMPL)]
    section_card(s, 0.35, 1.2, 12.6, 5.6, "已实现 9 项", [
        *impl_lines,
        "",
        "覆盖类型：反射率定标示意 · 立方体清洗/特征 · 植被指数 · SVM 分类 · 地块汇总",
        "其余 36 项：目录与 POST /api/v1/{id}/run 已预留（implemented=false），可按场景优先级补齐。",
    ...
    section_card(s, 0.35, 1.2, 12.6, 5.6, "本材料覆盖的三部分", [
        ...
        "2. 算法：45 项业界清单，逐项说明方法、输入输出、价值与流程位置。",
