...
def validate_content(algos: list[dict]) -> None:
    ...
    assert numbers == list(range(1, 56)), f"算法编号异常：{numbers}"
    assert len(ALG_IDS) == len(set(ALG_IDS)) == 55, "algorithm_id 缺失或重复"
    assert len(IMPL) == 55, f"可运行算法数量异常：{len(IMPL)}"
    assert len(set(range(1, 56)) - IMPL) == 0, "骨架算法数量异常"
    print("内容校验通过：55 项；可运行 55；骨架 0")
...
def build():
    ...
    add_text(
        ...
        [
            ...
            ("内容口径：55 项能力覆盖 L0–L4；全部可运行并产出 files", 12, False, (171, 207, 232)),
    ...
    s = blank(prs)
    track(s, "服务对接")
    title_header(s, "当前可运行能力（可联调）", "55 项全部可运行，统一 POST /api/v1/{id}/run")
    section_card(s, 0.35, 1.2, 7.65, 5.6, "可运行 55 项", [
